...
df=pd.read_csv('data_out_10M.csv', sep=',',low_memory=False)
#print(df.head())
dfcities=pd.read_csv('worldcities.csv', sep=',',low_memory=False)
...
def generateprofile(df,Mean,StDev):
    ...
    #user_id
    usid = random.randint(10000000,50000000)
    df3['user_id']=usid
...
def generateprofile1(df,Mean,StDev):
    ...
    # Session_id
    ses = random.randint(100000000,500000000)
    df3['Session_id']=ses

    # Customer_id
    cus= random.randint(1000000000,5000000000)
    df3['Customer_id']=cus
...
res=pd.DataFrame(columns=list(df.columns))
p=0.00001
pp=0.5
...
for i in range(int(numfake1/2)):
    ...
    res=res.append(ligne,ignore_index = True)
for i in range(int(numfake1/2)):
...
res.to_csv('fake_data.csv', index=False)
